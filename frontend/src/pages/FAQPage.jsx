        ...
        {faqs.map((faq) => (
          <div key={faq.id} className="bg-white rounded-2xl border border-cream-200 overflow-hidden shadow-xs">
            <button
              ...
              className="w-full p-5 text-left font-serif font-bold text-sm text-bakery-dark flex items-center justify-between hover:bg-cream-50"
            >
              <span>{faq.question}</span>
              <ChevronDown className={`w-4 h-4 text-bakery-caramel transition-transform ${openId === faq.id ? 'rotate-180' : ''}`} />
            </button>
            {openId === faq.id && (
              <div className="px-5 pb-5 text-xs text-gray-600 border-t border-cream-100 pt-3">
                {faq.answer}
              </div>
            )}
          </div>
